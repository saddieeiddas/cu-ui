      ...
      <div className={`build-panel ${this.state.minimized ? 'minimized' : ''}`}>
        ...
        {this.props.panesState.panes.map((rowPanes: BuildPane[], row: number) => {
          return (<div className='row' key={row}>
            ...
            {
              React.createElement(rowPanes[this.props.panesState.activeIndices[row]].component, {data: rowPanes[this.props.panesState.activeIndices[row]].data, minimized: this.state.minimized})
            }          
            </div>
          </div>
          )})}
